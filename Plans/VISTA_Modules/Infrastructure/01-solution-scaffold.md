...
VISTA is a WPF desktop application built in VB.NET 10 on .NET 10 following a **modular monolith** architecture. The solution contains one WPF startup project and five class libraries (SharedKernel + 4 business modules).
...
### WPF Startup Project (`MerchSys.App`)
...
- No WPF dependency — pure .NET library
...
5. Running `MerchSys.App` launches a blank WPF window
