...
| `src/MerchSys.App/Views/Accounting/IncomeStatementView.xaml` | `IncomeStatementView` | `.xaml.vb` | Displays the P&L statement supporting monthly, quarterly, and annual periods. CSV export via `SaveFileDialog`; PDF export generates a QuestPDF A4 document and opens `ReportPreviewWindow` with page images for preview, Save As PDF, and Print (ACC-23). Injects `IIncomeStatementPdfExporter` via DI. |
