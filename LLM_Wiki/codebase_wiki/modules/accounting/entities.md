...
| `src/MerchSys.Accounting/Entities/TamperAuditEntry.vb` | `TamperAuditEntry` | (None) | Append-only audit ledger for POS tamper incidents. Deviates from `AuditableEntity` (no `IsDeleted`, no `ModifiedBy`). |
