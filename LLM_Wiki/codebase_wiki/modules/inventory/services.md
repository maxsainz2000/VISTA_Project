...
| `src/MerchSys.Inventory/Services/IShrinkageService.vb`<br>`src/MerchSys.Inventory/Services/ShrinkageService.vb` | `IShrinkageService`<br>`ShrinkageService` | Inventory shrinkage recording (damage, spoilage). `RecordShrinkageAsync()`, `GetShrinkageHistoryAsync()`, `GetShrinkageHistoryPageAsync()` (INFRA-34). Now writes `StockMovement` records (Shrinkage) for all events. |
