...
| `src/MerchSys.Inventory/Entities/SaleCogsRecord.vb` | `SaleCogsRecord` | `Entity` | `TransactionId`, `ProductId`, `BatchId`, `QuantityDeducted`, `UnitCost`, `Cogs`, `DeductedAt`. Represents an append-only per-batch FIFO COGS record for a completed POS transaction. |
