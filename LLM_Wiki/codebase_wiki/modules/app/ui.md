...
| `src/MerchSys.App/Views/Purchasing/PurchaseOrderListView.xaml`<br>`src/MerchSys.App/Views/Purchasing/PurchaseOrderListView.xaml.vb` | `PurchaseOrderListView` | Active and historical purchase orders tracking. Sequential `TabIndex`; order editor overlay focused via `IsVisibleChanged` dispatcher (vendor combo box); `PreviewKeyDown` routes Esc→`CancelEditorCommand` / Enter→`SaveDraftCommand` (gated on overlay visibility); mnemonics on primary actions (UX-17). PO editor fields (Vendor, Expected Delivery, Notes) wrapped in `FieldRowStyle`; `ErrorSummary` control added (UX-18). | `PurchaseOrderListViewModel` (Constructor Injection) |
| `src/MerchSys.App/Views/Purchasing/PurchasingDashboardView.xaml`<br>`src/MerchSys.App/Views/Purchasing/PurchasingDashboardView.xaml.vb` | `PurchasingDashboardView` | Read-only landing dashboard for the Purchasing module (UX-13). Aggregates metrics, spends, top vendors, and pending deliveries. | `PurchasingDashboardViewModel` (Constructor Injection) |
