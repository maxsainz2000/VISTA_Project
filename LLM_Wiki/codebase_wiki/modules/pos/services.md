...
|---|---|---|---|
| `src/MerchSys.POS/Services/ICartService.vb`<br>`src/MerchSys.POS/Services/CartService.vb` | `ICartService`<br>`CartService` | `CreateCartAsync()`, `AddLineAsync()`, `UpdateLineQuantityAsync()`, `RemoveLineAsync()`, `ApplyLineDiscountAsync()`, `FinalizeAsync()`, `VoidTransactionAsync()`, `GetTransactionHistoryAsync()`, `GetTransactionHistoryPageAsync()`, `GetCartAsync()` | `POSDbContext`, `IReceiptService`, `VatConfigurationLoader` |
