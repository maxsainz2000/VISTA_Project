...
related: [client-server-wpf, modular-monolith, offline-first-sync, system-plan-amendment-2026-05-28]
...
VISTA is deployed as a pure client-server WPF system. Every client laptop connects directly, over TCP, to a single centralized **MariaDB 11.4.x LTS** instance hosted via XAMPP on a designated host laptop on the local network. No client maintains a local database; there is no sync layer.
...
- Each client opens an EF Core `DbContext` per unit of work (request-scoped in the WPF DI container).
